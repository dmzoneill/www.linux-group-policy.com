...
class GDLinkedInBadge {
    ...
    var $default_options = array(
            ...
            "text" => "Milan Petrovic",
            "description" => "If you want to see my LinkedIn profile, click on this button:",
            "badge" => 0,
            "image" => ""
        );
    ...
    function admin_head() {
        ...
        if ($parent_file == "widgets.php" || $parent_file == "themes.php") {
            ...
            for ($i = 0; $i < count($this->badges); $i++) {
                ...
                if ($i<count($this->badges)-1) $gdlb_js_items.=",";
            }
    ...
    function widget($args) {
        ...
        echo $this->make_badge($url, $text, $badge);
        echo $after_widget;
    }
    ...
    function make_badge($badge_url, $badge_text, $badge_id = 0) {
        $linked_img = "http://www.linkedin.com/img/webpromo/";
        $badge_array = $this->badges[$badge_id];
        return '<div style="text-align: center;"><a href="'.$badge_url.'"><img src="'.$linked_img.$badge_array[1].'" border="0" alt="'.$badge_text.'" /></a></div>';
    }
