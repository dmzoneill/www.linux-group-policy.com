...
function twitter_messages($username = '', $num = 1, $list = false, $update = true, $linked  = '#', $hyperlinks = true, $twitter_users = true, $encode_utf8 = false) {
	...
	if ($username == '') {
		if ($list) echo '<li>';
		_e('RSS not configured', TDOMAIN);
		if ($list) echo '</li>';
	} else {
			if ( empty($messages->items) ) {
				if ($list) echo '<li>';
				 _e('Twitter messages not available.', TDOMAIN);
				if ($list) echo '</li>';
			} else {
        $i = 0;
        ...
        if($update) {				
          ...
          if ( ( abs( time() - $time) ) < 86400 )
            $h_time = sprintf( __('%s ago', TDOMAIN), human_time_diff( $time ) );
          else
            $h_time = date(__('Y/m/d', TDOMAIN), $time);

          echo sprintf( __('%s', TDOMAIN),' <span class="twitter-timestamp"><abbr title="' . date(__('Y/m/d H:i:s', TDOMAIN), $time) . '">' . $h_time . '</abbr></span>' );
         }          
                  
					if ($list) echo '<span class="outside_quote">&ldquo;</span></li>'; elseif ($num != 1) echo '</p>';
...
function widget_twitter_init() {
  ...
  if ($check_options['number']=='') {
	...
	function widget_twitter_control($number) {
		...
		foreach($twitter_options['widget_fields'] as $key => $field) {
			...
			printf('<p style="text-align:right;" class="twitter_field"><label for="%s">%s <input id="%s" name="%s" type="%s" value="%s" class="%s" %s /></label></p>',
				$field_name, __($field['label'], TDOMAIN), $field_name, $field_name, $field['type'], $field_value, $field['type'], $field_checked);
		}
	...
	?>
		<div class="wrap">
			<form method="POST">
				<h2><?php _e('Twitter Widgets', TDOMAIN); ?></h2>
				<p style="line-height: 30px;"><?php _e('How many Twitter widgets would you like?', TDOMAIN); ?>
				<select id="twitter-number" name="twitter-number" value="<?php echo $options['number']; ?>">
	<?php for ( $i = 1; $i < 10; ++$i ) echo "<option value='$i' ".($options['number']==$i ? "selected='selected'" : '').">$i</option>"; ?>
				</select>
				<span class="submit"><input type="submit" name="twitter-number-submit" id="twitter-number-submit" value="<?php echo attribute_escape(__('Save', TDOMAIN)); ?>" /></span></p>
			</form>
		</div>
	<?php
	}
	...
	function widget_twitter_register() {
		...
		for ($i = 1; $i <= 9; $i++) {
			$name = sprintf(__('Twitter #%d', TDOMAIN), $i);
